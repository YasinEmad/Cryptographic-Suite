...
export const CryptoView: React.FC<CryptoViewProps> = ({
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-3xl font-bold text-brand-primary">{selectedAlgorithm.name}</h1>
        <p className="text-brand-secondary mt-2">{selectedAlgorithm.description}</p>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div>
           <label className="block text-sm font-bold text-brand-secondary mb-2">Input / Plaintext</label>
           <textarea
            value={inputText}
            onChange={(e) => setInputText(e.target.value)}
            placeholder="Enter text to process..."
            className="w-full h-48 p-3 bg-brand-surface rounded-md border-2 border-transparent focus:border-brand-primary focus:outline-none focus:ring-0 transition-colors font-mono"
          />
        </div>
        <div className="relative">
          ...
          <textarea
            ...
            className="w-full h-48 p-3 bg-brand-surface rounded-md border-2 border-transparent font-mono"
          />
      ...
      {error && (
          <div className="p-3 bg-brand-error/20 text-brand-error border border-brand-error/50 rounded-md">
